...
SceneGear::~SceneGear()
{
	...
	// サウンドを止める
	SoundManager::GetInstance().DesignationStopSound("GearScene");
}

void SceneGear::Init()
{
	// サウンドを流す
	SoundManager::GetInstance().Play("GearScene", false);
}
...
std::shared_ptr<SceneBase> SceneGear::Update()
{
	...
	// 左ボタンが押されたら列カウントを減らす
	if (Pad::IsTrigger(PAD_INPUT_LEFT)) m_rowCount--;
...
void SceneGear::Draw()
{
	...
	DrawFormatString(0, 0, 0xffffff, "%d", m_rowCount);
}
