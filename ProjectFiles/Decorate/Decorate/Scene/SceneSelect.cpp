...
namespace
{
	...
	constexpr int kStageSelectTextPosX = static_cast<int>(Game::kScreenWidth * 0.2f);		// ステージ選択テキスト座標X
	constexpr int kStageSelectTextPosY = static_cast<int>(Game::kScreenHeight * 0.05f);		// ステージ選択テキスト座標Y
	constexpr int kSelectBoxPosX = static_cast<int>(Game::kScreenWidth * 0.155f);			// ステージ選択ボックス座標X
	constexpr int kSelectBoxPosY = static_cast<int>(Game::kScreenHeight * 0.25f);			// ステージ選択ボックス座標Y
	constexpr int kLeftArrowPosX = static_cast<int>(Game::kScreenWidth * 0.03f);			// 左矢印座標X
	constexpr int kRightArrowPosX = static_cast<int>(Game::kScreenWidth * 0.86f);			// 右矢印座標X
	constexpr int kArrowPosY = static_cast<int>(Game::kScreenHeight * 0.5f);				// 矢印座標Y
	...
	const std::string kSelectTextPath = kSelectPath + "StageSelect/Text.png";		// 選択時テキスト画像パス
...
{
	// テキスト描画
	DrawGraph(kStageSelectTextPosX, kStageSelectTextPosY,
		m_selectTextH, true);
...
void SceneSelect::ImgLoad()
{
	...
	assert(m_decisionButtonH != -1);
	m_selectTextH = LoadGraph(kSelectTextPath.c_str());
	assert(m_selectTextH != -1);
